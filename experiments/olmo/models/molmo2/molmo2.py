...
from olmo.models.molmo2.molmo2_preprocessor import (
...
class Molmo2Config(BaseModelConfig):
    ...
    """Whether to add LoRA adapters."""

    lora_rank: Optional[int] = None
    """Rank of LoRA adapters."""

    lora_alpha: Optional[int] = None
    """Alpha of LoRA adapters."""

    lora_dropout: Optional[float] = None
    """Dropout of LoRA adapters."""

    lora_bias: Optional[str] = None
    """Bias setting for LoRA adapters."""
...
class Molmo2(ModelBase):
    ...
    def _functional_masked_add(
        ...
        update_indices = torch.nonzero(flat_mask, as_tuple=False).squeeze(-1)
        if update_indices.numel() == 0:
            return hidden_states
        updates = added_features.reshape(-1, added_features.shape[-1]).to(dtype=flat_hidden.dtype)
        if updates.shape[0] != update_indices.shape[0]:
            raise ValueError(
                f"Masked add shape mismatch: {updates.shape[0]=} vs {update_indices.shape[0]=}"
            )
        flat_hidden = torch.index_add(flat_hidden, 0, update_indices, updates)
    ...
    def num_params_action_expert(self, include_embedding: bool = True, include_inactive_params: bool = True) -> int:
        ...
        params = (
            ...
            if named_param[0].startswith("action_expert.")
        )
        return sum(p.numel() for _, p in params)
    ...
    def forward(
        ...
        **kwargs,
        ...
        enable_cp = (self._cp_load_balancer is not None) and (
            not (use_cache or collect_layer_kv_states)
        )  # keep action KV capture off the decode-cache path
            ...
            if past_key_values is not None:
                if bool(c.any()):
                    raise NotImplementedError(
                        "Cached decode with new image tokens is not supported when "
        ...
        if subsegment_ids is not None:
            # We only need to forbid the incremental-cache path with packed segments.
            # Full-sequence forwards may still request cache outputs for auxiliary heads.
            assert past_key_values is None, "Subsegment_ids cannot be used with cached past_key_values."
        ...
        if images is not None:
            if low_res_token_pooling is None:
                ...
                if any(is_high_res_patch):
                    x = self._functional_masked_add(x, is_high_res_patch, image_features)
                    ...
                    x = self._functional_masked_add(x, is_image_patch, image_features)
                    if deepstack_image_features is not None:
                ...
                for i, input_id in enumerate([self._image_low_res_id, self._image_high_res_id]):
                    ...
                    x = self._functional_masked_add(
                        x,
                        is_image_patch,
                        image_features.view(-1, image_features.shape[-1])[mask.view(-1)],
                    )
        ...
        attn_key_values: Optional[List[Tuple[torch.Tensor, torch.Tensor]]] = (
            [] if (use_cache or collect_layer_kv_states) else None
        )
        all_hidden_states = []
        ...
        for block_idx, block in enumerate(self.transformer.blocks):
            ...
            x, cache = block(
                ...
                use_cache=use_cache,
                collect_kv_states=collect_layer_kv_states,
                **all_block_kwargs,
                **block_kwargs,
            )
            if deepstack_image_features is not None and block_idx in range(len(deepstack_image_features)):
                for i, (is_image_patch, mask) in enumerate(zip(image_pos_masks, deepstack_masks)):
                    ...
                    x = self._functional_masked_add(x, is_image_patch, added_features)
    ...
    def generate(
        ...
        target_end_index = llm_cfg.build_tokenizer().eos_token_id if end_index is None else int(end_index)

        if return_conditioning and beam_size != 1:
            raise NotImplementedError("return_conditioning=True currently requires beam_size=1.")

        if beam_size == 1 and (return_conditioning or end_index is not None):
            if sampler is not None or constraints:
                raise NotImplementedError(
                    "Custom end-index greedy generation does not support sampler/constraints."
                )
            ...
            collected_layer_hidden_states: Optional[List[List[torch.Tensor]]] = (
                [] if return_conditioning else None
            )
            final_attn_key_values: Optional[List[Tuple[torch.Tensor, torch.Tensor]]] = None
            current_input_ids = input_ids
            current_past_key_values: Optional[List[Tuple[torch.Tensor, torch.Tensor]]] = None
            current_position_ids = position_ids
            current_append_last_valid_logits = append_last_valid_logits
            current_image_args = image_args
            current_attention_mask = attention_mask
            current_attention_bias = attention_bias
            scores: Optional[torch.Tensor] = None
            consumed_final_token = False

            def _record_generation_output(output: OLMoOutput) -> None:
                nonlocal collected_layer_hidden_states, final_attn_key_values
                final_attn_key_values = output.attn_key_values
                if not return_conditioning:
                    return
                layer_hidden_states = None if output.internal is None else output.internal.get("layer_hidden_states")
                if layer_hidden_states is None:
                    raise RuntimeError("Generation conditioning requested but no layer hidden states were returned.")
                if collected_layer_hidden_states is None:
                    collected_layer_hidden_states = [[] for _ in range(len(layer_hidden_states))]
                for layer_idx, layer_hidden in enumerate(layer_hidden_states):
                    collected_layer_hidden_states[layer_idx].append(layer_hidden)

            def _build_next_position_ids(pos_ids: torch.Tensor) -> torch.Tensor:
                return pos_ids[:, -1:] + 1

            def _consume_token_for_conditioning(
                token_ids: torch.Tensor,
                past_key_values: Optional[List[Tuple[torch.Tensor, torch.Tensor]]],
                attention_mask: Optional[torch.Tensor],
                position_ids: Optional[torch.Tensor],
            ) -> tuple[OLMoOutput, Optional[torch.Tensor]]:
                next_input_ids = token_ids.unsqueeze(1)
                next_attention_mask = attention_mask
                if not llm_cfg.use_position_ids and next_attention_mask is not None:
                    next_attention_mask = torch.cat(
                        (
                            next_attention_mask,
                            next_attention_mask.new_ones((next_input_ids.shape[0], 1)),
                        ),
                        dim=-1,
                    )
                if llm_cfg.use_position_ids and position_ids is not None:
                    next_position_ids = _build_next_position_ids(position_ids)
                else:
                    next_position_ids = None
                consumed_output = self(
                    next_input_ids,
                    attention_mask=next_attention_mask,
                    attention_bias=current_attention_bias,
                    position_ids=next_position_ids,
                    past_key_values=past_key_values,
                    use_cache=True,
                    last_logits_only=True,
                    append_last_valid_logits=None,
                    collect_layer_hidden_states=return_conditioning,
                )
                _record_generation_output(consumed_output)
                return consumed_output, next_attention_mask
            ...
            with torch.inference_mode(), torch.compiler.set_stance("force_eager"):
                for step_idx in range(max_steps):
                    output = self(
                        ...
                        use_cache=True,
                        last_logits_only=True,
                        append_last_valid_logits=current_append_last_valid_logits,
                        collect_layer_hidden_states=return_conditioning,
                        **current_image_args,
                    )
                    _record_generation_output(output)
                    log_probs = F.log_softmax(output.logits[:, -1, :], dim=-1)
                    next_token = torch.argmax(log_probs, dim=-1)
                    next_scores = torch.gather(log_probs, 1, next_token.unsqueeze(-1)).squeeze(-1)
                    generated_tokens.append(next_token)
                    scores = next_scores if scores is None else (scores + next_scores)

                    stop_now = step_idx + 1 >= min_steps if min_steps is not None else True
                    stop_now = bool(stop_now) and bool((next_token == target_end_index).all())
                    if stop_now:
                        if include_final_token_in_conditioning:
                            output, current_attention_mask = _consume_token_for_conditioning(
                                next_token,
                                output.attn_key_values,
                                current_attention_mask,
                                current_position_ids,
                            )
                            consumed_final_token = True
                        break

                    current_input_ids = next_token.unsqueeze(1)
                    current_past_key_values = output.attn_key_values
                    current_image_args = {}
                    current_append_last_valid_logits = None
                    if not llm_cfg.use_position_ids and current_attention_mask is not None:
                        current_attention_mask = torch.cat(
                            (current_attention_mask, current_attention_mask.new_ones((current_input_ids.shape[0], 1))),
                            dim=-1,
                        )
                    if llm_cfg.use_position_ids and current_position_ids is not None:
                        current_position_ids = _build_next_position_ids(current_position_ids)
                    else:
                        current_position_ids = None

                if (
                    generated_tokens
                    and include_final_token_in_conditioning
                    and not consumed_final_token
                ):
                    output, current_attention_mask = _consume_token_for_conditioning(
                        generated_tokens[-1],
            ...
            token_ids = torch.stack(generated_tokens, dim=-1).unsqueeze(1)
            scores = scores.unsqueeze(1) if scores is not None else input_ids.new_zeros((batch_size, 1), dtype=torch.float32)
            ...
            if return_conditioning:
                ...
                if collected_layer_hidden_states is not None:
                    hidden_states = tuple(
                        torch.cat(layer_parts, dim=1) for layer_parts in collected_layer_hidden_states
                    )
                if current_attention_mask is not None:
                    final_attention_mask = current_attention_mask[:, : full_token_ids.shape[-1]]
                else:
                    final_attention_mask = full_token_ids[:, 0, :] != -1

            return OLMoGenerateOutput(
                token_ids=token_ids,
                scores=scores.to(dtype=torch.float32),
                full_token_ids=full_token_ids,
                attn_key_values=final_attn_key_values,
