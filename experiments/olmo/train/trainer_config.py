...
class CheckpointLoadStrategy(StrEnum):
    auto = "auto"
    """
    Use rank0-only I/O plus broadcast for unsharded checkpoints and the distributed
    checkpoint loader for sharded checkpoints.
    """

    rank0_broadcast = "rank0_broadcast"
    """
    Force rank0-only I/O followed by a broadcast-based restore path. This is only
    valid for unsharded checkpoints.
    """

    distributed_sharded = "distributed_sharded"
    """
    Force the distributed sharded checkpoint loader. This is only valid for sharded
    checkpoints.
    """
...
class FSDPConfig(BaseConfig):
    ...
    def get_fsd2_args(self, autocast_precision) -> Dict:
        ...
        if self.hybrid_sharding_num_model_replicas:
            num_model_replicas = self.hybrid_sharding_num_model_replicas
            if get_world_size() % num_model_replicas != 0:
                raise OLMoConfigurationError("fsdp.hybrid_sharding_num_model_replicas must divide world size")
...
class TrainConfig(BaseConfig):
    ...
    ft_action_expert: bool = True
    """
    Tune the action expert parameters.
    """

    # Do we fine-tune the input/output embeddings
    ft_embedding: str = "lm_head"
    """
    Tune the embedding layers. Supported values include:
    - "all": do not freeze embedding-related layers
    - "ln_f": only tune final norm
    - "lm_head": tune final norm + output head
    - "wte": tune token embeddings
    - "added_tokens": tune final norm + output head, and only added-token rows in wte.embedding
    """
    ...
    vlm_data: Optional[DataLoaderConfig] = None
    """
    Optional dedicated VLM training dataloader configuration.
    """
    ...
    Retained only so older saved configs still deserialize cleanly.
    """

    action_loader_rate: Optional[float] = None
    """
    Deprecated. Legacy config field for action-loader interleaving.
    Retained only so older saved configs still deserialize cleanly.
    """
    ...
    """
    Save inloop prediction into a json file in the save_folder
    """
    ...
    save_merged_lora_checkpoint: bool = False
    """
    When LoRA is enabled, additionally save a merged unsharded checkpoint
    ...
    checkpoint_load_strategy: CheckpointLoadStrategy = CheckpointLoadStrategy.auto
    """
    Strategy used when loading checkpoints at startup. In ``auto`` mode unsharded
    checkpoints use rank0-only I/O plus broadcast while sharded checkpoints use the
    ...
    distributed_eval: Optional[bool] = True
    """
    Run inference evaluation with a non-distributed model
    
    This will load a non-distributed model onto the GPU, so inference will require more memory as 
    and some initial setup time.
    
    However this can be much faster for inf. evals that require generating many tokens
    """
