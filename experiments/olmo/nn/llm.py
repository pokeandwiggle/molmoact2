...
from olmo.config import BaseConfig, StrEnum, TransformerDataParallelWrappingStrategy
from olmo.safetensors_util import safetensors_file_to_state_dict
from olmo.tokenizer import (
    TokenizerConfig,
    DEFAULT_PAD_MULTIPLE,
)
from olmo.torch_util import ensure_finite_, BufferCache, get_global_rank, barrier
from olmo.nn.cp_load_balancer import CPLoadBalancerType
from olmo.nn.ulysses import UlyssesAttention
from olmo.util import resource_path, rank0_resource_path
from olmo.dist_util import get_dp_mesh
...
class LlmConfig(BaseConfig):
    ...
    def build_tokenizer(self):
        total_added_tokens = len(self.tokenizer.resolve_new_tokens_for_both_input_and_output())
        if total_added_tokens > 0:
            pad_tokenizer_to = max(
                int(self.embedding_size or self.vocab_size),
                int(self.vocab_size + total_added_tokens),
            )
            # Keep padded rows aligned for added-token runs.
            if DEFAULT_PAD_MULTIPLE > 1:
                pad_tokenizer_to = (
                    ((pad_tokenizer_to + DEFAULT_PAD_MULTIPLE - 1) // DEFAULT_PAD_MULTIPLE)
                    * DEFAULT_PAD_MULTIPLE
                )
        elif self.fix_pad_tokenizer:
    ...
    @property
    def mlp_dim(self):
        return self.mlp_hidden_size if self.mlp_hidden_size is not None else self.mlp_ratio * self.d_model
        ...
        if tokenizer_cfg is not None:
            for key in ("add_action_tokens", "num_action_tokens", "num_additional_action_tokens"):
                if key in tokenizer_cfg:
                    log.warning(
                        "Removing deprecated tokenizer key '%s' from %s.tokenizer.",
                        key,
                        cls.__name__,
                    )
...
class Llm(nn.Module):
    def __init__(self, config: LlmConfig, cache, device):
        ...
        self.token_bias: Optional[nn.Module] = None
        ...
        if self.config.init_path is None:
            ...
            if is_sharded:
                ...
                if self.config.init_incremental:
                    log.warning(
                        "Ignoring llm.init_incremental=%r and using a single broadcast-based "
                        "checkpoint load for reliability.",
                        self.config.init_incremental,
                    )
...
class Embedding(nn.Module):
    ...
    def forward(self, x: torch.Tensor, logits=False, logits_with_new_embedding=False) -> torch.Tensor:
        if logits_with_new_embedding:
            return F.linear(x, torch.cat([self.embedding, self.new_embedding], dim=0))
        if logits:
...
class FixedTokenBias(nn.Module):
    """Fixed additive token bias for a contiguous token-id range."""
    ...
    def __init__(
        ...
        hidden_size: int,
        scale: float,
        init_table: torch.Tensor,
    ):
        super().__init__()
        if init_table.shape != (num_tokens, hidden_size):
            raise ValueError(
                f"Expected init_table shape {(num_tokens, hidden_size)}, got {tuple(init_table.shape)}"
            )
        self.token_start_id = int(token_start_id)
        self.num_tokens = int(num_tokens)
        self.scale = float(scale)
        self.register_buffer("bias_table", init_table.to(dtype=torch.float32), persistent=False)

    def forward(self, input_ids: torch.Tensor, dtype: Optional[torch.dtype] = None) -> torch.Tensor:
        token_indices = input_ids - self.token_start_id
        token_mask = (token_indices >= 0) & (token_indices < self.num_tokens)
        safe_token_indices = token_indices.clamp(min=0, max=self.num_tokens - 1)
        bias = F.embedding(safe_token_indices, self.bias_table)
        if dtype is not None and bias.dtype != dtype:
            bias = bias.to(dtype=dtype)
        bias = bias * token_mask.unsqueeze(-1).to(dtype=bias.dtype)
        return bias * self.scale


class CompositeTokenBias(nn.Module):
    """Additive composition of multiple fixed token bias modules."""

    def __init__(self, *bias_modules: nn.Module):
        super().__init__()
        self.bias_modules = nn.ModuleList([m for m in bias_modules if m is not None])

    def append(self, bias_module: nn.Module) -> None:
        self.bias_modules.append(bias_module)

    def forward(self, input_ids: torch.Tensor, dtype: Optional[torch.dtype] = None) -> torch.Tensor:
        if len(self.bias_modules) == 0:
            raise RuntimeError("CompositeTokenBias requires at least one child module.")
        total = self.bias_modules[0](input_ids, dtype=dtype)
        for module in self.bias_modules[1:]:
            total = total + module(input_ids, dtype=dtype)
        return total
...
class RotaryEmbedding(nn.Module):
    ...
    def warmup_cache(self, device, cp_enabled: bool = False):
        if self.config.max_sequence_length is not None:
            if self.config.full_attention_layers is not None and self.config.rope_type != RopeType.default:
                assert not cp_enabled, "cp_enabled not supported with mixed RoPE types"
    ...
    def get_cp_rotary_embedding(self, seq_len: int, device: torch.device) -> Tuple[torch.Tensor, torch.Tensor]:
        if (
            (pos_sin := self.__cache.get("cp_rope_pos_sin")) is not None
            and (pos_cos := self.__cache.get("cp_rope_pos_cos")) is not None
            and pos_sin.shape[-2] >= seq_len
            and pos_cos.shape[-2] >= seq_len
        ):
            if pos_sin.device != device:
                ...
                # but it is hard to pin down why its sometimes needed and sometimes isn't
                if not torch.compiler.is_compiling():
                    self.__cache["cp_rope_pos_sin"] = pos_sin
            if pos_cos.device != device:
                pos_cos = pos_cos.to(device)
                if not torch.compiler.is_compiling():
                    self.__cache["cp_rope_pos_cos"] = pos_cos
...
class OLMoBlock(nn.Module):
    ...
    def cp_attention(
        ...
        q, k, v = self._prepare_cp_attention_qkv(
            q,
            k,
            v,
            position_ids=position_ids,
    ...
    def attention(
        ...
        B, _, C = q.size()  # batch size, sequence length, d_model
        q, k, v = self._prepare_attention_qkv(
            q,
            k,
            v,
            position_ids=position_ids,
...
class OLMoEBlock(OLMoBlock):
    ...
    def forward(
        ...
        collected_cache = None
        if collect_kv_states and not use_cache:
            _, cache_k, cache_v = self._prepare_attention_qkv(
                q,
                k,
                v,
                position_ids=position_ids,
            ...
            collected_cache = (cache_k, cache_v)
...
class OLMoSequentialBlock(OLMoBlock):
    ...
    def forward(
        ...
        #  - for group query attn q: (batch_size, seq_len, d_model)
        #                      k, v: (batch_size, seq_len, d_model // n_kv_heads)
        enable_cp = getattr(self, '_cp_enabled', False) and not (use_cache or collect_kv_states)
        if not self.config.norm_after:
            if self.fine_grained_checkpoint_fn is not None:
